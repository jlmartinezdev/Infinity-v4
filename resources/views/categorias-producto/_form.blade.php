...
<div class="space-y-6">
    <div>
        ...
        <input type="text" name="nombre" id="nombre" value="{{ old('nombre', $categoriaProducto?->nombre) }}"
            ...
            maxlength="150" required autofocus placeholder="Ej: Electrónica, Accesorios">
        @error('nombre')
    ...
    <div>
        ...
        <textarea name="descripcion" id="descripcion" rows="3"
            class="w-full px-4 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 shadow-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20 focus:outline-none transition-colors resize-y">{{ old('descripcion', $categoriaProducto?->descripcion) }}</textarea>
        @error('descripcion')
            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
        @enderror
    </div>
    ...
    <div class="flex flex-wrap gap-3">
        <button type="submit"
            ...
            {{ $categoriaProducto ? 'Actualizar categoría' : 'Crear categoría' }}
        </button>
