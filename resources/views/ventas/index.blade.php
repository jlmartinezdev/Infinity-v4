...
<div class="max-w-7xl mx-auto">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow border border-gray-200 dark:border-gray-700 overflow-hidden">
        <form method="GET" action="{{ route('ventas.index') }}" class="p-4 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700/50">
            <div class="flex flex-col sm:flex-row gap-3 flex-wrap">
                ...
                <div class="sm:w-56">
                    <select name="cliente_id" class="w-full py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 shadow-sm focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20 focus:outline-none transition-colors bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100">
                        <option value="">Todos los clientes</option>
                        @foreach($clientes as $cli)
                            <option value="{{ $cli->cliente_id }}" {{ request('cliente_id') == $cli->cliente_id ? 'selected' : '' }}>{{ $cli->nombre }} {{ $cli->apellido }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="sm:w-40">
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead class="bg-gray-50 dark:bg-gray-700/50">
                    <tr>
                        ...
                        <th scope="col" class="px-4 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Cobrado</th>
                ...
                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                    @forelse ($ventas as $venta)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                            <td class="px-4 py-3 text-sm text-gray-900 dark:text-gray-100">{{ $venta->id }}</td>
                            <td class="px-4 py-3 text-sm text-gray-900 dark:text-gray-100">{{ $venta->fecha?->format('d/m/Y') ?? '—' }}</td>
                            <td class="px-4 py-3 text-sm text-gray-900 dark:text-gray-100 font-medium">{{ $venta->cliente ? trim($venta->cliente->nombre . ' ' . $venta->cliente->apellido) : '—' }}</td>
                            <td class="px-4 py-3 text-sm text-gray-900 dark:text-gray-100">{{ $venta->numero_factura ?? '—' }}</td>
                            <td class="px-4 py-3 text-sm text-right text-gray-900 dark:text-gray-100 font-medium">{{ number_format($venta->total ?? 0, 2, ',', '.') }}</td>
                            <td class="px-4 py-3 text-sm text-right text-gray-900 dark:text-gray-100">{{ number_format($venta->cobrado ?? 0, 2, ',', '.') }}</td>
                            ...
                            <td class="px-4 py-3 text-right text-sm">
                                <a href="{{ route('ventas.show', $venta) }}" class="text-purple-600 dark:text-purple-400 hover:text-purple-800 dark:hover:text-purple-300 font-medium mr-4">Ver</a>
                                <a href="{{ route('ventas.edit', $venta) }}" class="text-purple-600 dark:text-purple-400 hover:text-purple-800 dark:hover:text-purple-300 font-medium mr-4">Editar</a>
                                <form action="{{ route('ventas.destroy', $venta) }}" method="POST" class="inline" onsubmit="return confirm('¿Eliminar esta venta?');">
                                    @csrf
                                    @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="px-4 py-8 text-center text-gray-500 dark:text-gray-400">No hay ventas. <a href="{{ route('ventas.create') }}" class="text-purple-600 dark:text-purple-400 hover:underline">Registrar una</a>.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
