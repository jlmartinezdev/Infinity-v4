...
@section('title', 'Editar marca OLT')
...
<div class="max-w-2xl mx-auto">
    <div class="mb-6">
        ...
        <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100 mt-1">Editar marca: {{ $oltMarca->nombre }}</h1>
    </div>

    <form action="{{ route('sistema.olt-marcas.update', $oltMarca) }}" method="POST" class="bg-white dark:bg-gray-800 rounded-xl shadow border border-gray-200 dark:border-gray-700 p-6">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-4">
            <div>
                ...
                <input type="text" name="nombre" id="nombre" value="{{ old('nombre', $oltMarca->nombre) }}" required maxlength="100"
            ...
            <div>
                ...
                <select name="estado" id="estado" class="mt-1 w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100">
                    <option value="activo" {{ old('estado', $oltMarca->estado) === 'activo' ? 'selected' : '' }}>Activo</option>
                    <option value="inactivo" {{ old('estado', $oltMarca->estado) === 'inactivo' ? 'selected' : '' }}>Inactivo</option>
                </select>
            </div>
            <div>
                <label for="notas" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Notas</label>
                <textarea name="notas" id="notas" rows="2" class="mt-1 w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100">{{ old('notas', $oltMarca->notas) }}</textarea>
            </div>
        </div>
