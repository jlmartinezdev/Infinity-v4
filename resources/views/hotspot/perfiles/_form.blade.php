...
<div class="space-y-4">
    ...
    <div>
        ...
        @error('rate_limit')
            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
        @enderror
        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Velocidad MikroTik. Vacío = sin tope de ancho de banda.</p>
    </div>

    <div>
        <label for="cuota_gb" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Cuota de datos (GB)</label>
        <input type="number" name="cuota_gb" id="cuota_gb" value="{{ old('cuota_gb', $perfil?->cuota_gb) }}"
            class="w-full px-4 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20"
            min="1" max="10240" step="1" placeholder="Ej: 10">
        @error('cuota_gb')
            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
        @enderror
        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Tope acumulado de tráfico (subida + bajada) por usuario. Vacío = ilimitado. 1 GB = 1024 MB.</p>
    </div>
