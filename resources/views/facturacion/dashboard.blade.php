...
<div class="max-w-7xl mx-auto">
    ...
    <div id="ingresos-nodo" class="bg-white dark:bg-gray-800 rounded-xl shadow border border-gray-200 dark:border-gray-700 p-4 sm:p-6 mt-6">
        <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-4">
            <div>
                <h2 class="text-lg font-semibold text-gray-900 dark:text-gray-100">Plata ingresada por nodo</h2>
                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                    Cobros de {{ $mesNodoEtiqueta }} por fecha de pago, atribuidos al nodo del servicio del cliente.
                </p>
            </div>
            <form method="get" action="{{ route('facturacion.dashboard') }}#ingresos-nodo" class="flex items-end gap-2 shrink-0">
                <div>
                    <label for="mes" class="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Mes</label>
                    <select name="mes" id="mes" onchange="this.form.submit()"
                        class="rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 text-sm px-3 py-2 focus:ring-2 focus:ring-emerald-500">
                        @foreach($opcionesMesNodo as $opcion)
                            <option value="{{ $opcion['valor'] }}" @selected($mesNodoSeleccionado === $opcion['valor'])>{{ $opcion['etiqueta'] }}</option>
                        @endforeach
                    </select>
                </div>
            </form>
        ...
        <p class="text-sm text-gray-600 dark:text-gray-400 mb-4">
            Total del mes:
            <span class="font-semibold text-emerald-700 dark:text-emerald-400">{{ number_format((float) $totalIngresoNodo, 0, ',', '.') }} PYG</span>
        </p>
        @if(($seriesIngresoNodo ?? collect())->isNotEmpty())
        <div class="w-full" style="height: {{ max(280, min(560, 48 * max(count($seriesIngresoNodo), 1))) }}px;">
            <canvas id="facturacionIngresoNodoChart"></canvas>
        </div>
        @endif
        <div class="overflow-x-auto mt-4">
            <table class="min-w-full text-sm divide-y divide-gray-200 dark:divide-gray-700">
                <thead class="bg-gray-50 dark:bg-gray-700/50">
                    <tr>
                        <th class="px-3 py-2 text-left font-medium text-gray-600 dark:text-gray-300">Nodo</th>
                        <th class="px-3 py-2 text-right font-medium text-gray-600 dark:text-gray-300">Recibos</th>
                        <th class="px-3 py-2 text-right font-medium text-gray-600 dark:text-gray-300">Ingresado</th>
                        <th class="px-3 py-2 text-right font-medium text-gray-600 dark:text-gray-300">%</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    @forelse(($seriesIngresoNodo ?? []) as $row)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/40">
                            <td class="px-3 py-2 text-gray-900 dark:text-gray-100">{{ $row['nodo'] }}</td>
                            <td class="px-3 py-2 text-right text-gray-700 dark:text-gray-300">{{ number_format((int) ($row['cantidad'] ?? 0), 0, ',', '.') }}</td>
                            <td class="px-3 py-2 text-right text-emerald-700 dark:text-emerald-400 font-medium">{{ number_format((float) ($row['total'] ?? 0), 0, ',', '.') }} PYG</td>
                            <td class="px-3 py-2 text-right text-gray-500 dark:text-gray-400">{{ number_format((float) ($row['porcentaje'] ?? 0), 1, ',', '.') }}%</td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="4" class="px-3 py-6 text-center text-gray-500 dark:text-gray-400">Sin cobros en el mes seleccionado.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
...
<script>
    (function () {
        ...
        const dataIngresoNodo = @json($seriesIngresoNodo);
        ...
        const labelsIngresoNodo = dataIngresoNodo.map(item => item.nodo);
        const valuesIngresoNodo = dataIngresoNodo.map(item => Number(item.total || 0));
        ...
        const elIngresoNodo = document.getElementById('facturacionIngresoNodoChart');
        if (elIngresoNodo) {
            const scalesNodo = T.chartAxisTheme ? T.chartAxisTheme(null) : { x: {}, y: {} };
            scalesNodo.x = scalesNodo.x || {};
            scalesNodo.y = scalesNodo.y || {};
            scalesNodo.x.beginAtZero = true;
            scalesNodo.x.ticks = scalesNodo.x.ticks || {};
            scalesNodo.x.ticks.callback = function (value) { return formatPYG(value); };
            scalesNodo.y.ticks = scalesNodo.y.ticks || {};
            scalesNodo.y.ticks.autoSkip = false;
            delete scalesNodo.y.beginAtZero;
            if (scalesNodo.y.ticks.callback) delete scalesNodo.y.ticks.callback;

            const ingresoNodoChart = new Chart(elIngresoNodo, {
                type: 'bar',
                data: {
                    labels: labelsIngresoNodo,
                    datasets: [
                        {
                            label: 'Plata ingresada',
                            data: valuesIngresoNodo,
                        ...
                        tooltip: {
                            callbacks: {
                                label: function (context) {
                                    ...
                                    return (context.dataset.label || '') + ': ' + formatPYG(value);
                                }
                            }
                        }
                    },
        ...
        if (T.watchThemeCharts) {
            T.watchThemeCharts(charts, charts.map(() => function (value) {
                return formatPYG(value);
            }));
        }
    })();
