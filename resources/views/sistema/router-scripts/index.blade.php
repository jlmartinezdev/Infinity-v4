...
@section('title', 'Scripts MikroTik')

@section('content')
<div class="max-w-7xl mx-auto">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100">Scripts MikroTik</h1>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Leé scripts de un router, guardalos en BD y sincronizalos a otro</p>
        </div>
        <div class="flex flex-wrap gap-2">
            <a href="{{ route('sistema.router-schedulers.index') }}"
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow border border-gray-200 dark:border-gray-700 overflow-hidden">
        <form method="GET" action="{{ route('sistema.router-scripts.index') }}" class="p-4 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700/50">
            <div class="flex flex-col sm:flex-row gap-3">
                <div class="flex-1">
                    <input type="text" name="buscar" value="{{ request('buscar') }}"
                        placeholder="Buscar por nombre, notas o contenido..."
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead class="bg-gray-50 dark:bg-gray-700/50">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Leído</th>
                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Sync</th>
                    ...
                    @forelse($scripts as $s)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                            <td class="px-4 py-3">
                                ...
                                @endif
                                <div class="text-[11px] text-gray-400 mt-0.5">{{ number_format(mb_strlen($s->source)) }} chars</div>
                            </td>
                            ...
                            <td class="px-4 py-3 text-sm text-gray-600 dark:text-gray-400 whitespace-nowrap">
                                {{ $s->leido_en?->format('d/m/Y H:i') ?? '—' }}
                            </td>
                            <td class="px-4 py-3 text-sm text-gray-600 dark:text-gray-400 whitespace-nowrap">
                                {{ $s->sincronizado_en?->format('d/m/Y H:i') ?? '—' }}
                            </td>
                            <td class="px-4 py-3 text-right text-sm whitespace-nowrap">
                                <button type="button"
                                    class="btn-sync-script text-green-600 dark:text-green-400 hover:underline font-medium mr-3"
                                    data-id="{{ $s->router_script_id }}"
                                    data-nombre="{{ $s->nombre }}"
                                    data-url="{{ route('sistema.router-scripts.sync', $s) }}">
                                    Sync a router
                                </button>
                                <a href="{{ route('sistema.router-scripts.edit', $s) }}" class="text-purple-600 dark:text-purple-400 hover:underline font-medium mr-3">Editar</a>
                                <form action="{{ route('sistema.router-scripts.destroy', $s) }}" method="POST" class="inline" onsubmit="return confirm('¿Eliminar este script de la BD?');">
                                    @csrf
                                    @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-4 py-10 text-center text-sm text-gray-500 dark:text-gray-400">
                                No hay scripts guardados. Usá «Leer desde router» para importar.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    <div class="flex min-h-full items-end sm:items-center justify-center p-4">
        ...
        <div class="relative w-full max-w-3xl rounded-xl bg-white dark:bg-gray-800 shadow-xl border border-gray-200 dark:border-gray-700">
            <div class="flex items-start justify-between gap-3 p-4 border-b border-gray-200 dark:border-gray-700">
                <div>
                    <h2 class="text-lg font-semibold text-gray-900 dark:text-gray-100">Leer scripts desde router</h2>
            ...
            <div class="p-4 space-y-4">
                ...
                <div id="import-lista" class="max-h-80 overflow-y-auto rounded-lg border border-gray-200 dark:border-gray-700 divide-y divide-gray-100 dark:divide-gray-700">
                    <p class="p-4 text-sm text-gray-500">Elegí un router y listá sus scripts.</p>
                </div>
            </div>
...
<div id="modal-sync" class="hidden fixed inset-0 z-50 overflow-y-auto" aria-modal="true" role="dialog">
    <div class="flex min-h-full items-end sm:items-center justify-center p-4">
        ...
        <div class="relative w-full max-w-lg rounded-xl bg-white dark:bg-gray-800 shadow-xl border border-gray-200 dark:border-gray-700">
            <div class="flex items-start justify-between gap-3 p-4 border-b border-gray-200 dark:border-gray-700">
                <div>
                    <h2 class="text-lg font-semibold text-gray-900 dark:text-gray-100">Sincronizar script</h2>
            ...
            <div class="p-4 space-y-3">
                ...
                <p class="text-xs text-gray-500 dark:text-gray-400">Si el script ya existe en el destino (mismo nombre), se actualiza el source.</p>
            </div>
...
(function () {
    ...
    var listUrl = @json(route('sistema.router-scripts.list-remote'));
    var importUrl = @json(route('sistema.router-scripts.import'));
    ...
    document.getElementById('btn-abrir-importar')?.addEventListener('click', function () {
        open(modalImport);
    });
    ...
    document.getElementById('btn-listar-remotos')?.addEventListener('click', function () {
        ...
        fetch(listUrl + '?router_id=' + encodeURIComponent(routerId), {
            ...
            .then(function (res) {
                ...
                if (!scripts.length) {
                    lista.innerHTML = '<p class="p-4 text-sm text-gray-500">Este router no tiene scripts en /system/script.</p>';
                    return;
                }
                lista.innerHTML = scripts.map(function (s) {
                    return '<label class="flex items-start gap-3 p-3 hover:bg-gray-50 dark:hover:bg-gray-700/40 cursor-pointer">'
                        ...
                        + '<span class="block text-xs text-gray-500 mt-0.5">' + (s.source_length || 0) + ' chars'
                        + (s.owner ? ' · owner ' + s.owner : '')
                        + (s.policy ? ' · ' + s.policy : '')
                        + '</span>'
                        + (s.source_preview ? '<span class="block text-[11px] text-gray-400 mt-1 truncate">' + s.source_preview + '</span>' : '')
                        + '</span></label>';
                }).join('');
    ...
    btnGuardar?.addEventListener('click', function () {
        ...
        if (!routerId) { alert('Seleccioná un router.'); return; }
        if (!nombres.length) { alert('Seleccioná al menos un script.'); return; }
        var btn = this;
        btn.disabled = true;
        fetch(importUrl, {
            method: 'POST',
    ...
    document.querySelectorAll('.btn-sync-script').forEach(function (btn) {
    ...
    document.getElementById('btn-confirmar-sync')?.addEventListener('click', function () {
        ...
        if (!confirm('¿Escribir este script en el router destino?')) return;
        var btn = this;
        btn.disabled = true;
        fetch(syncUrl, {
            method: 'POST',
