...
@section('title', 'Catálogo OLT')

@section('content')
<div class="max-w-7xl mx-auto">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100">Catálogo OLT</h1>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Modelos con imagen para seleccionar en los equipos OLT</p>
        </div>
        <div class="flex flex-wrap gap-2">
            <a href="{{ route('sistema.olts.index') }}"
            ...
            <a href="{{ route('sistema.olt-modelos.create') }}"
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow border border-gray-200 dark:border-gray-700 overflow-hidden mb-6">
        <form method="GET" action="{{ route('sistema.olt-modelos.index') }}" class="p-4 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700/50">
            <div class="flex flex-col lg:flex-row gap-3">
                <div class="flex-1">
                    <input type="text" name="buscar" value="{{ request('buscar') }}"
                        placeholder="Buscar por nombre, slug, marca..."
                ...
                <div class="sm:w-44">
                    <select name="marca" class="w-full py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 shadow-sm focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20 focus:outline-none transition-colors bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100">
    ...
    @if($modelos->isEmpty())
        <div class="rounded-xl border border-dashed border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 p-12 text-center">
            ...
            <a href="{{ route('sistema.olt-modelos.create') }}" class="mt-3 inline-block text-purple-600 dark:text-purple-400 hover:underline font-medium">Agregar el primero</a>
        </div>
    @else
                ...
                <article class="group flex flex-col overflow-hidden rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-sm hover:shadow-md hover:border-purple-300 dark:hover:border-purple-700 transition-all {{ !$m->activo ? 'opacity-75' : '' }}">
                    ...
                    <div class="flex flex-1 flex-col p-4">
                        ...
                        <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">
                            {{ $m->olts_count }} {{ $m->olts_count === 1 ? 'OLT' : 'OLTs' }}
                        </p>

                        <div class="mt-auto pt-4 flex flex-wrap gap-2">
                            <a href="{{ route('sistema.olt-modelos.edit', $m) }}"
                            ...
                            <form action="{{ route('sistema.olt-modelos.destroy', $m) }}" method="POST" class="inline"
